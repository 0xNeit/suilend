...
In Suilend, the interest rate is variable and also compounded every second. Therefore, every second, the variable APR (aka borrow rate) must be recalculated. 

In Suilend, the variable APR is calculated as a function of reserve utilization. 

Formally, say there is a total of X tokens borrowed from the SUI reserve at $t=0$. How much has that debt compounded by $t=1$?
